...
import { redeem } from '../../../services/pointService';
...
export default function DetailPromo() {
  ...
  useEffect(() => {
    Promise.all([getRewards(), getMemberProfile()])
      .then(([rewardsRes, profileRes]) => {
        if (rewardsRes.success) {
          const found = rewardsRes.data.map(mapReward).find((r) => String(r.id) === String(id));
          setReward(found || null);
        }
        if (profileRes.success) setCurrentBalance(profileRes.data.totalPoints || 0);
      })
  ...
  const canRedeem = currentBalance >= reward.pointRequired;
  ...
  const handleConfirmRedeem = async () => {
    try {
      ...
      setShowConfirm(false);
      alert(err.message || 'Gagal menukar reward.');
    }
  };
        ...
        <div className="flex gap-3">
          {(reward.benefits || []).map((b) => (
            <div key={b.title} className="flex-1 bg-[#F7F9FB] border border-[#E0E3E5]/50 rounded-xl p-4 flex flex-col gap-1">
              <div className="w-[18px] h-[18px] bg-primary rounded-sm" />
